/**
 * CRAFT - Main Interpreter
 * Dalvik bytecode execution loop.
 */

import { Value, NULL_VALUE } from '../core/types';
import { DexParser } from '../parser/dex_parser';
import { Heap } from './heap';
import { FrameManager, ExecutionFrame } from './frame';
import { ClassLoader } from './class_loader';
import { OpcodeTable, ExecutionContext, InterpreterControl } from './opcode_table';
import { ShimRegistry, InterpreterRef, ClassLoaderRef } from './shim_registry';
import { ResolvedMethod } from './types';
import { registerEssentialOpcodes } from './opcodes';
import {
  InterpreterError,
  NoSuchMethodError,
  AbstractMethodError,
...
export class Interpreter implements InterpreterControl, InterpreterRef {
  ...
  invoke(
    ...
    if (!method.code) {
      throw new AbstractMethodError(
        `${method.classDescriptor}.${method.name}`
      );
    }
  ...
  invokeMethod(method: ResolvedMethod, args: Value[]): void {
    if (method.isShim) {
      // Execute shim immediately
      this.lastResult = this.shimRegistry.invoke(
        method,
        this,
        this.heap,
        args
  ...
  private executeInstruction(frame: ExecutionFrame): void {
    ...
    if (frame.pc >= insns.length) {
      throw new InterpreterError(
        `PC out of bounds: ${frame.pc} >= ${insns.length} in ${frame.method.classDescriptor}.${frame.method.name}`
      );
    }
