...
export function accessFlagsToString(flags: number, isMethod: boolean = false): string {
    ...
    if (flags & AccessFlags.ACC_PUBLIC) parts.push('PUBLIC');
    if (flags & AccessFlags.ACC_PRIVATE) parts.push('PRIVATE');
    if (flags & AccessFlags.ACC_PROTECTED) parts.push('PROTECTED');
    if (flags & AccessFlags.ACC_STATIC) parts.push('STATIC');
    if (flags & AccessFlags.ACC_FINAL) parts.push('FINAL');
    
    if (isMethod) {
        if (flags & AccessFlags.ACC_SYNCHRONIZED) parts.push('SYNCHRONIZED');
        if (flags & AccessFlags.ACC_BRIDGE) parts.push('BRIDGE');
        if (flags & AccessFlags.ACC_VARARGS) parts.push('VARARGS');
    } else {
        if (flags & AccessFlags.ACC_VOLATILE) parts.push('VOLATILE');
        if (flags & AccessFlags.ACC_TRANSIENT) parts.push('TRANSIENT');
    }
    
    if (flags & AccessFlags.ACC_NATIVE) parts.push('NATIVE');
    if (flags & AccessFlags.ACC_INTERFACE) parts.push('INTERFACE');
    if (flags & AccessFlags.ACC_ABSTRACT) parts.push('ABSTRACT');
    if (flags & AccessFlags.ACC_STRICT) parts.push('STRICT');
    if (flags & AccessFlags.ACC_SYNTHETIC) parts.push('SYNTHETIC');
    if (flags & AccessFlags.ACC_ANNOTATION) parts.push('ANNOTATION');
    if (flags & AccessFlags.ACC_ENUM) parts.push('ENUM');
    if (flags & AccessFlags.ACC_CONSTRUCTOR) parts.push('CONSTRUCTOR');
    if (flags & AccessFlags.ACC_DECLARED_SYNCHRONIZED) parts.push('DECLARED_SYNCHRONIZED');
