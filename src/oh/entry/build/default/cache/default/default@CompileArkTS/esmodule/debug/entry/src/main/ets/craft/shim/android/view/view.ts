...
import { NULL_VALUE, intValue, objectRef } from "@bundle:com.craft.runtime/entry/ets/craft/core/types";
...
export function registerViewShim(registry: ShimRegistry, uiBridge?: UIBridge): void {
    ...
    registry.register(VIEW_CLASS, 'setOnClickListener', '(Landroid/view/View$OnClickListener;)V', (interp, heap, thisRef, args) => {
        ...
        if (uiBridge && listenerRef.type === 'object' && listenerRef.ref !== 0) {
            ...
            const callback = (): void => {
                ...
                if (listenerClass) {
                    interp.invoke(listenerClass, 'onClick', '(Landroid/view/View;)V', [capturedListenerRef, objectRef(capturedThisRef)]);
    ...
    // performClick()Z
    registry.register(VIEW_CLASS, 'performClick', '()Z', (interp, heap, thisRef, _args) => {
        ...
        if (listenerRef.type === 'object' && listenerRef.ref !== 0) {
            ...
            if (listenerClass) {
                interp.invoke(listenerClass, 'onClick', '(Landroid/view/View;)V', [listenerRef, objectRef(thisRef)]);
                return intValue(1);
            }
        }
        return intValue(0);
