...
import { NoSuchMethodError } from "@bundle:com.craft.runtime/entry/ets/craft/interpreter/errors";
...
export type ShimMethod = (interpreter: InterpreterRef, heap: Heap, thisRef: number, args: Value[]) => Value;
export class ShimRegistry {
    ...
    invoke(method: ResolvedMethod, interpreter: InterpreterRef, heap: Heap, args: Value[]): Value {
        const key = `${method.classDescriptor}:${method.name}:${method.descriptor}`;
        const handler = this.methods.get(key);
        if (!handler) {
            throw new NoSuchMethodError(`No shim for ${method.classDescriptor}.${method.name}${method.descriptor}`);
        }
