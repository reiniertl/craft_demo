...
import type { ClassLoader } from './class_loader';
...
export class OpcodeTable {
    ...
    execute(ctx: ExecutionContext, opcode: number, insn: number): void {
        ...
        if (!info) {
            throw new InterpreterError(`Unimplemented opcode: 0x${opcode.toString(16).padStart(2, '0')} at PC=${ctx.frame.pc}`);
        }
...
export function getRegisters_35c(insn: number, word2: number): number[] {
    ...
    if (count >= 1)
        regs.push(word2 & 0xf);
    if (count >= 2)
        regs.push((word2 >> 4) & 0xf);
    if (count >= 3)
        regs.push((word2 >> 8) & 0xf);
    if (count >= 4)
        regs.push((word2 >> 12) & 0xf);
    if (count >= 5)
        regs.push((insn >> 8) & 0xf);
    return regs;
}
