...
import { NULL_VALUE, objectRef } from "@bundle:com.craft.runtime/entry/ets/craft/core/types";
...
export function registerContextShim(registry: ShimRegistry): void {
    // Context.<init>()V
    registry.register(CONTEXT_CLASS, '<init>', '()V', (_interp, _heap, _thisRef, _args) => {
        return NULL_VALUE;
    });
    // Context.getApplicationContext()Landroid/content/Context;
    registry.register(CONTEXT_CLASS, 'getApplicationContext', '()Landroid/content/Context;', (_interp, _heap, thisRef, _args) => {
...
export function registerContextWrapperShim(registry: ShimRegistry): void {
    ...
    registry.register(CONTEXT_WRAPPER_CLASS, '<init>', '()V', (_interp, _heap, _thisRef, _args) => {
        return NULL_VALUE;
    });
    // ContextWrapper.<init>(Landroid/content/Context;)V
    registry.register(CONTEXT_WRAPPER_CLASS, '<init>', '(Landroid/content/Context;)V', (_interp, heap, thisRef, args) => {
        heap.setField(thisRef, 'mBase', args[0]);
        return NULL_VALUE;
    });
    // ContextWrapper.getBaseContext()Landroid/content/Context;
    registry.register(CONTEXT_WRAPPER_CLASS, 'getBaseContext', '()Landroid/content/Context;', (_interp, heap, thisRef, _args) => {
        return heap.getField(thisRef, 'mBase');
    });
    // ContextWrapper.getApplicationContext()Landroid/content/Context;
    registry.register(CONTEXT_WRAPPER_CLASS, 'getApplicationContext', '()Landroid/content/Context;', (_interp, _heap, thisRef, _args) => {
        return objectRef(thisRef);
    });
}
