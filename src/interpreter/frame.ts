/**
 * CRAFT - Execution Frame Management
 * Register-based execution frames for the Dalvik VM.
 */

import { Value, intValue, NULL_VALUE } from '../core/types';
import { ResolvedMethod, isWideValue } from './types';
...
export class FrameManager {
  ...
  pushFrame(frame: ExecutionFrame): void {
    if (this.stack.length >= this.maxStackDepth) {
      throw new InterpreterError(
        `Stack overflow: exceeded max depth of ${this.maxStackDepth}`
      );
    }
  ...
  getStackTrace(): string[] {
    ...
    for (let i = this.stack.length - 1; i >= 0; i--) {
      ...
      trace.push(
        `  at ${frame.method.classDescriptor}.${frame.method.name}${frame.method.descriptor} (pc=${frame.pc})`
      );
    }
    return trace;
  }
